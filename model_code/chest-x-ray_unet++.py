...
#### 1) 데이터 로드
X_train = np.load('Chest-X-Ray/npy/chest_X_train.npy')
y_train = np.load('Chest-X-Ray/npy/chest_y_train.npy')
X_val = np.load('Chest-X-Ray/npy/chest_X_val.npy')
y_val = np.load('Chest-X-Ray/npy/chest_y_val.npy')
X_test = np.load('Chest-X-Ray/npy/chest_X_test.npy')
y_test = np.load('Chest-X-Ray/npy/chest_y_test.npy')
...
for i in range(len(y_test)):
    ...
    y_pred_i = y_test_pred_binary[i].squeeze().flatten()  # 변수명 수정: y_pred_thresh → y_test_pred_binary
...
# 저장 디렉토리 생성 후 CSV 저장
SAVE_PATH = 'Chest-X-Ray/results'
os.makedirs(SAVE_PATH, exist_ok=True)

csv_path = os.path.join(SAVE_PATH, 'per_sample_metrics.csv')
