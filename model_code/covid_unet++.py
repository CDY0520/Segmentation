...
X_train = np.load(os.path.join(base_dir, "X_train_1.npy"))
X_val = np.load(os.path.join(base_dir, "X_val_1.npy"))
X_test = np.load(os.path.join(base_dir, "X_test_1.npy"))
y_train = np.load(os.path.join(base_dir, "y_train_1.npy"))
y_val = np.load(os.path.join(base_dir, "y_val_1.npy"))
y_test = np.load(os.path.join(base_dir, "y_test_1.npy"))
...
#### 2. U-Net++ 모델 정의
smooth = 1.0
def unet_plus_plus(input_size=(224, 224, 1), deep_supervision=False):
...
model = unet_plus_plus(input_size=(224, 224, 1), deep_supervision=True)
...
# 데이터프레임을 CSV 파일로 저장
csv_file_path = "covid_unet++_dataframe.csv"
